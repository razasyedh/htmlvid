...
def get_args():
    ...
    config_files = [os.path.expanduser("~") + os.sep + ".htmlvid",
                    "~/.config/.htmlvid", WINDOWS_CONFIG]
    ...
    group0.add_argument("video", type=video_file,
                        help="The path to the video file. Use '-' to read from"
                             " stdin.")
    ...
    group1.add_argument("-v", "--verbose", action="count",
                        help="Use once to make htmlvid verbose, twice to show"
                             " FFmpeg output, and three times for both.")
    group1.add_argument("-V", "--version", action="version",
                        version="htmlvid %s" % __version__,
                        help="Print version number.")

    group2 = parser.add_argument_group("Time Options",)
    group2.add_argument("-s", "--start", metavar="time", type=time_stamp,
                        help="Time to start at in hh:mm:ss[.m...] format. The"
                             " m is for decimal seconds.")
    group2.add_argument("-t", "--to", metavar="dur", type=snip_duration,
                        help="The duration in seconds and decimal seconds. A"
    ...
    group4.add_argument("-b", "--bitrate", metavar="rate", type=filesize,
                        help="Maximum bitrate to use for WebM's. Must include"
                             " K or M suffix. Default: 750K.")
    ...
    group6.add_argument("-O", "--options", metavar="opts", type=ffmpeg_options,
                        help="Options to pass directly to FFmpeg. (Escape any"
                             " dashes and quote spaces for your shell. ie:"
                             " \\-foo 'bar baz')")
    group6.add_argument("-F", "--filters", metavar="filt",
                        help="Filters to pass to FFmpeg's '-vf' option"
                             " with commas separating them. Escape any special"
                             " characters.")
    ...
    verbose_print(os.linesep + parser.format_values(), args.verbose)
...
def error_out(short_arg, long_arg, message):
    ...
    error = "argument -%s/--%s: %s%s" % (short_arg, long_arg, message,
                                         os.linesep)
...
def set_implied(args):
    ...
    if args.metadata is None and args.video != "-":
    ...
    if args.output is None:
        ...
        args.output = "%s.%s" % (unix_time, args.format)
...
def escape_args(args):
    """Properly escape string literals for FFmpeg."""
    args.video = args.video.replace("'", "'\\''")
    args.output = args.output.replace("'", "'\\''")
    if args.metadata is not None:
        args.metadata = args.metadata.replace("'", "'\\''")
...
def timestamp_to_sec(timestamp):
    """Break up a timestamp and convert it to seconds."""
    # "hh" : "mm" : "ss[.m...]"
    hms = timestamp.split(":")
    sec = int(hms[0]) * 60 * 60 + int(hms[1]) * 60 + float(hms[2])
...
def calculate_duration(start, end):
    ...
    start_sec = timestamp_to_sec(start)
    end_sec = timestamp_to_sec(end)

    difference_sec = end_sec - start_sec
...
def calculate_bitrate(limit, duration):
    ...
    bitrate = str(round(bitrate, 2)) + suffix
    return bitrate
...
def get_full_duration(video_path):
    ...
    metadata = subprocess.check_output(cmd_list, stdin=sys_stdin)
...
def print_progress(log_path, duration):
    ...
    print "\rProgress: %.0f%% (%.3f/%.3fs)" % (percent, current_time,
                                               duration),
...
def run_cmd(command, input_video, duration, is_two_pass, verbosity):
    ...
    if not is_two_pass or is_first_pass:
        ...
        verbose_print(msg, verbosity)
        msg = "Filename: %s" % filename + os.linesep
        verbose_print(msg, verbosity)
    ...
    ffmpeg_process = subprocess.Popen(cmd_list, stdin=sys_stdin, cwd=temp_dir)
...
def main():
    ...
    if not args.testrun:
        ...
        run_cmd(command, args.video, args.to, args.twopass, args.verbose)

        if args.twopass:
            command2 = command.replace("-pass 1", "-pass 2")
            print "Running pass 2"

            run_cmd(command2, args.video, args.to, args.twopass, args.verbose)
